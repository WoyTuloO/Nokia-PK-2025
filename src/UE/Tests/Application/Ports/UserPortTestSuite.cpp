...
TEST_F(UserPortTestSuite, shallShowNewMessageReceived)
{
    EXPECT_CALL(loggerMock,
                log(common::ILogger::INFO_LEVEL, testing::HasSubstr("[USER-PORT]New Message has been received")));
...
TEST_F(UserPortTestSuite, shallShowListMessage)
{
    ...
    EXPECT_CALL(
        loggerMock,
        log(common::ILogger::INFO_LEVEL,
            testing::HasSubstr("[USER-PORT]Showing SMS List (Count: " + std::to_string(messages.size()) + ")")));
    EXPECT_CALL(guiMock, setListViewMode()).WillOnce(ReturnRef(listViewModeMock));
    EXPECT_CALL(listViewModeMock, clearSelectionList());
...
TEST_F(UserPortTestSuite, shallShowMessageView)
{
    ...
    EXPECT_CALL(loggerMock,
                log(common::ILogger::INFO_LEVEL,
                    testing::AnyOf(testing::HasSubstr("[USER-PORT]From: " + common::to_string(message.sender)),
    ...
    EXPECT_CALL(textModeMock, setText(testing::AnyOf(HasSubstr("From: 112"), HasSubstr("To: 112"))));
...
TEST_F(UserPortTestSuite, shallShowIncomingCall)
{
    ...
    EXPECT_CALL(loggerMock,
                log(common::ILogger::INFO_LEVEL,
                    testing::HasSubstr("Showing incoming call from: " + common::to_string(caller))));
    EXPECT_CALL(guiMock, setAlertMode()).WillOnce(ReturnRef(alertModeMock));
...
TEST_F(UserPortTestSuite, shallShowCallInProgress)
{
    ...
    EXPECT_CALL(loggerMock,
                log(common::ILogger::INFO_LEVEL,
                    testing::HasSubstr("Showing call in progress with: " + common::to_string(other))));
    EXPECT_CALL(guiMock, setAlertMode()).WillOnce(ReturnRef(alertModeMock));
...
TEST_F(UserPortTestSuite, shallShowEndedCall)
{
    ...
    EXPECT_CALL(loggerMock,
                log(common::ILogger::INFO_LEVEL,
                    testing::HasSubstr("Showing ended call with: " + common::to_string(other) + " reason: " + reason)));
...
TEST_F(UserPortTestSuite, shallShowCallFailed)
{
    ...
    EXPECT_CALL(loggerMock,
                log(common::ILogger::INFO_LEVEL,
                    testing::HasSubstr("Showing failed call with: " + common::to_string(other) + " error: " + error)));
...
TEST_F(UserPortTestSuite, shallShowAlertPeerUnknownRecipient)
{
    common::PhoneNumber other{ 123 };
    EXPECT_CALL(loggerMock,
                log(common::ILogger::INFO_LEVEL, testing::HasSubstr("BTS does not recognize this phone number")));
    EXPECT_CALL(guiMock, setAlertMode()).WillOnce(ReturnRef(alertModeMock));
